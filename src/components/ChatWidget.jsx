...
export default function ChatWidget({ embedConfig = { isEmbed: false, agentId: null } }) {
  ...
  // EMBED MODE: Render with transparent background, only button visible
  if (embedConfig.isEmbed) {
    return (
      <div style={{
        position: 'fixed',
        top: 0,
        ...
        {/* Launcher Button - always visible with pointer-events */}
        <button 
  ...
  // NON-EMBED MODE: Original rendering
  return (
    <>
      {isOpen && (
        <div style={{
          ...
          {/* Same header/messages/input as above - keeping DRY for now */}
          {/* (In production, you'd extract these to separate components) */}
        </div>
      )}
